...
## RBrain note (Hermes / cole-macbook / any other agent)

This skill is mirrored into RBrain so any agent that loads `skills/RESOLVER.md` discovers it. The actual harness is a separate repo (`https://github.com/browser-use/browser-harness`) that must be installed once per machine. On a new machine:

1. Read `install.md` (next to this file) and follow it. It clones the harness repo and installs `browser-harness` as a global `uv tool`.
2. Read `helpers.py` from the cloned harness repo (`~/Projects/browser-harness/helpers.py` on Ramy's MacBook; pick a stable path on cole-macbook). That file is the API surface.
3. After install succeeds, the rest of this SKILL.md is the runtime contract.

If `command -v browser-harness` already returns a path, skip step 1 and go straight to runtime usage.
