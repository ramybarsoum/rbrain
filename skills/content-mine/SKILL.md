...
- When triggered by cron or another autonomous workflow, only kick off the interview with Step 1 unless the user is actively replying in-session.
...
- Don't try to complete the full interview inside a cron job or any run that cannot wait for user replies.
