...
Do **not** read the full skill library on every task. Use `skills/RESOLVER.md` (or
`AGENTS.md`, accepted as of upstream v0.19) only when you need skill routing, filing
guidance, or a specific brain-writing workflow. Then load only the specific skill files
the current task needs.

GBrain + RBrain-fork ship 40+ skills organized by `skills/RESOLVER.md`:
