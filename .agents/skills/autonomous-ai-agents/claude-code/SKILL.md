...
## Running Installed Plugins / Plugin Skills From Hermes

When the user asks to "run" an installed Claude Code plugin, most plugins do not expose a standalone executable. They install skills and slash commands that Claude Code invokes inside a Claude session.

Reusable pattern:

1. Verify installation and enabled state:
   ```bash
   claude plugin list
   ```
2. Inspect plugin metadata if the invocation is unclear:
   ```bash
   find ~/.claude/plugins/cache -path '*<plugin-name>*' -maxdepth 8 -type f
   ```
   Then read the plugin `README.md`, `.claude-plugin/plugin.json`, or skill `SKILL.md` under the cache directory.
3. Run the plugin skill through Claude Code print mode with a natural-language prompt or explicit skill name:
   ```bash
   claude -p 'Use the <plugin-name> plugin / <skill-name> skill to ...' --allowedTools 'Read,Glob,Grep,Bash' --max-turns 12 --output-format json
   ```
4. For read-only analysis plugins, restrict tools to read-only/discovery tools (`Read,Glob,Grep,Bash`) and verify no repo changes afterward with `git status --short`.
5. Parse the JSON result and report the final `result` field, plus whether files changed.

Example from `claude-code-setup@claude-plugins-official`: it installs the skill `claude-code-setup:claude-automation-recommender`; running it successfully used:

```bash
claude -p 'Use the claude-code-setup plugin / claude-automation-recommender skill to analyze this project and recommend the top Claude Code automations. Keep it read-only.' --allowedTools 'Read,Glob,Grep,Bash' --max-turns 12 --output-format json
```
