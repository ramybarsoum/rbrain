...
```bash
$GBRIDGE sheets spreadsheets create --json '{"properties": {"title": "My Sheet"}}'
# Returns JSON with spreadsheetId and spreadsheetUrl
```

### Populating a Sheet with Data (values update)

**CRITICAL: Shell quoting breaks with complex JSON containing quotes, special characters, or large payloads.** Always write JSON to a temp file and read it from a Python subprocess instead of passing inline.

The `gws spreadsheets values update` command takes spreadsheet ID and range via `--params`, NOT via `--spreadsheet` flag (unlike the `+read` and `+append` helpers):

```python
import json, subprocess

body = json.dumps({"values": [["col1", "col2"], ["val1", "val2"]]})
params = json.dumps({
...
result = subprocess.run(
    ["hermes-agent/venv/bin/python", "skills/productivity/google-workspace/scripts/gws_bridge.py",
...
**Do NOT use the `google_api.py` wrapper** for bulk sheet writes with complex data. It fails on shell quoting with embedded quotes/commas. Use `gws_bridge.py` directly via Python subprocess.

### Batch Formatting a Sheet

Use `spreadsheets batchUpdate` with `--params` for the spreadsheet ID. Format properties MUST be nested under `userEnteredFormat`, not placed directly on the cell:
...
# WRONG — will get schema validation error
{"repeatCell": {
    "cell": {"textFormat": {"bold": True}},  # NOT a valid cell property
    "fields": "textFormat"
}}
```
...
subprocess.run([GBRIDGE, "sheets", "spreadsheets", "batchUpdate",
